...
public class ArtPollReplyPacket extends ArtNetPacket {
    ...
    private static final int ARTPOLLREPLY_LENGTH = 239;
    ...
    public ArtPollReplyPacket() {
        ...
        setData(new byte[ARTPOLLREPLY_LENGTH]);
        setHeader();
        setProtocol();
        try {
            InetAddress a = InetAddress.getLocalHost(); //TODO do something better here
            data.setByteChunk(a.getAddress(),10,4);
        } catch (UnknownHostException e) {
            e.printStackTrace();
        }
        data.setInt16(ArtNetServer.DEFAULT_PORT, 14);
        data.setInt16(1, 172); //Num ports
        data.setInt16(65535, 20);
        data.setInt8(0xF0, 23);

        byte[] name = new byte[]{'A','r','t','N','e','t','-','A','v','r'};
        data.setByteChunk(name,26,name.length);
        data.setByteChunk(name,44,name.length);
        data.setByteChunk(name,108,name.length);

        data.setByteChunk(new byte[]{(byte) 0x45, (byte) 0x45, (byte) 0x45, (byte) 0x45}, 174, 4);
        data.setByteChunk(new byte[]{1,2,3,4},186,4);
        data.setByteChunk(new byte[]{5,6, 7, 8}, 190,4);
        data.setInt8(1,194);
    }
